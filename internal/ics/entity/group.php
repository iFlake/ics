...
class Group
{
    ...
    public function SetPermission($name, $value, $id)
    {
        ...
        {
            if ($this->GetRawPermission($name, $id) == \itais\ics\permission\Permission::inherit) $table_group_permissions->Insert(["uid" => $this->uid, "name" => $name, "value" => $value, "id" => $id]);
            else $table_group_permissions->Update(["name" => $name, "value" => $value, "id" => $id], "uid = {$this->uid} and name = '" . $itais_ics_database_connection->escape_string($name) . "' and id = {$id}");
        }
    }
