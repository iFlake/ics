...
class User
{
    ...
    public function SetPermission($name, $value, $id = -1)
    {
        ...
        {
            if ($this->GetRawPermission($name, $id) == \itais\ics\permission\Permission::inherit) $table_user_permissions->Insert(["uid" => $this->uid, "name" => $name, "value" => $value, "id" => $id]);
            else $table_user_permissions->Update(["name" => $name, "value" => $value, "id" => $id], "uid = {$this->uid} and name = '" . $itais_ics_database_connection->escape_string($name) . "' and id = {$id}");
        }
    }
